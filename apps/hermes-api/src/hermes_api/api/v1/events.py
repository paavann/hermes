...
async def get_events(
    ...
    stmt = select(
        Event,
        select(StoryAllowlist.id)
        .where(StoryAllowlist.event_id == Event.id)
        .exists()
        .label("has_lineage"),
    )
    if status:
...
async def get_event_lineage(
    ...
    if not node_ids:
        ...
        raise HTTPException(
            status_code=404, detail="event not found or no lineage available."
        )
    ...
    edges_stmt = select(EventEdge).where(
        EventEdge.event_a_id.in_(node_ids), EventEdge.event_b_id.in_(node_ids)
    )
