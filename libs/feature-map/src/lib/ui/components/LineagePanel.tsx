...
export function LineagePanel({ map }: { map: mapboxgl.Map | null }) {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useEventLineage(lineageTargetId);
  ...
  useEffect(() => {
    ...
    if (!map.getSource('lineage-edges-source')) {
      map.addSource('lineage-edges-source', {
        type: 'geojson',
        data: edgesGeoJson,
      });
    } else {
      (map.getSource('lineage-edges-source') as mapboxgl.GeoJSONSource).setData(
        edgesGeoJson,
      );
    }

    if (!map.getSource('lineage-nodes-source')) {
      map.addSource('lineage-nodes-source', {
        type: 'geojson',
        data: nodesGeoJson,
      });
    } else {
      (map.getSource('lineage-nodes-source') as mapboxgl.GeoJSONSource).setData(
        nodesGeoJson,
      );
    }
    ...
    return () => {
      ...
      if (map.getSource('lineage-nodes-source'))
        map.removeSource('lineage-nodes-source');
      if (map.getSource('lineage-edges-source'))
        map.removeSource('lineage-edges-source');
    };
  ...
  return (
    <div className="absolute top-0 left-0 h-screen w-96 bg-hud-bg/95 backdrop-blur-md border-r border-hud-border flex flex-col font-mono z-20 shadow-[0_0_30px_rgba(0,0,0,0.8)]">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-4">
        {isLoading && (
          <div className="text-neon-blue text-xs tracking-widest animate-pulse">
            RECONSTRUCTING TIMELINE...
          </div>
        )}
        {isError && (
          <div className="text-red-500 text-xs tracking-widest">
          ...
          return (
            ...
            >
              ...
              <div className="text-[10px] text-gray-400 mb-1 flex justify-between">
                ...
                {node.location_name && (
                  <span className="text-neon-blue truncate max-w-[120px]">
                    {node.location_name}
                  </span>
                )}
              </div>
